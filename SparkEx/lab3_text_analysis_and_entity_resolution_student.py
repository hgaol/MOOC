...
def simpleTokenize(string):
    ...
    ret = []
    string = string.lower()
    sp_str = re.split(split_regex, string)
    for w in sp_str:
        if w == '':
            continue
        ret.append(w);
...
def idfs(corpus):
    ...
    tokenCountPairTuple = uniqueTokens.flatMap(lambda x: ((item,1) for item in x)).reduceByKey(lambda x, y: x + y)
...
def tfidf(tokens, idfs):
    ...
    tfIdfDict = {}
    for item in tokens:
        tfIdfDict[item] = tfs[item] * idfs[item]
    return tfIdfDict
