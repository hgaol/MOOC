...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    x1 = np.sum(X ** 2, axis=1, keepdims=True)
    x2 = np.sum(self.X_train ** 2, axis=1)
    x3 = -2 * np.dot(X, self.X_train.T)
    dists = x1 + x3
    dists = np.sqrt(dists + x2)
    #print x1.shape, x2.shape, x3.shape, dists.shape
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      I = np.argsort(dists[i,:])
      closest_y = self.y_train[I[:k]]
      #print closest_y
      ...
      for item in labels_dict:
          if labels_dict[item] > max:
              max = labels_dict[item]
              y_pred[i] = item
      #########################################################################
      #                           END OF YOUR CODE                            #
      #########################################################################
